...
impl Parser {
    ...
    fn expect_name(&mut self, what: &str) -> Result<String, String> {
        match self.take() {
    ...
    fn type_system(&mut self, keyword: String) -> Result<Definition, String> {
        match keyword.as_str() {
            "schema" => {
                self.directives()?;
    ...
    fn directives(&mut self) -> Result<(), String> {
        while self.eat('@') {
    ...
    fn roots(&mut self) -> Result<Vec<(String, String)>, String> {
        if !self.eat('{') {
    ...
    fn fields(&mut self, typed: bool) -> Result<Vec<String>, String> {
        if !self.eat('{') {
    ...
    fn balanced(&mut self, open: char, close: char) -> Result<(), String> {
        if !self.eat(open) {
